...
kknn_c$NA_INTERSALT = kknn_c$NA_INTERSALT/kknn_c$K_KAWASAKI
...
kknn_c$na_int_binary = ifelse(kknn_c$NA_INTERSALT < 1, "group1","group2")
...
#dilution regression
##calculate lambda
Kdata=read.table("KAWASAKI_PAHO_50w.txt",head=T)
nd= which(!is.na(kknn_c$data.INTERSALT_wK2))
cal_dil = data.frame(first = kknn_c$NA_INTERSALT[nd],second = kknn_c$data.INTERSALT_wK2[nd]/Kdata$data.KAWASAKI_K2[nd])
...
second_mean_group1 <- mean(cal_dil$second[which(cal_dil$first_group=="group1")],na.rm=T)
second_mean_group5 <- mean(cal_dil$second[which(cal_dil$first_group=="group5")],na.rm=T)
...
# # pdf("PH_major_3.pdf",width = 10,height = 10)
...
sen3_a = list(sm5.1,sm5.2,sm5.3,sm5.4,lambda)
save(sen3_a,file = paste("sen3_res",i,".Rdata",sep=""))
