...
####################################################
##c.d.adjust for medication history：BMI1+waist_circumference+diabetes_check+
#CVD_check+stroke_check + eGFR_cyst+b (proportional hazards cox)
...
a = list(sm1.1,sm1.2,sm1.3,sm1.4,sm3.1,sm3.2,sm3.3,sm3.4,sm5.1,sm5.2,
         sm5.3,sm5.4,lambda)
save(a,file=paste("sen_rm_hypertension_res",i,".Rdata",sep=""))
}


#KDIGO
kknn_c
sum(kknn_c$event.type[which(data$group=="group1")])/sum(data$time[which(data$group=="group1")]/365)*100000
incidence <- function(data){
  a = round(sum(data$event.type)/sum(data$follow_up_time_minor1/365)*100000,3)
  b = sum(data$event.type)
  c = dim(data)[1]
  d = paste(b,"/",c,sep="")
  out = data.frame(a,d)
  return(out)
}
data11 = kknn_c[which(kknn_c$eGFR_cyst>=90 & kknn_c$UACR_cat==0),]
data12 = kknn_c[which(kknn_c$eGFR_cyst>=90 & kknn_c$UACR_cat==1),]
data13 = kknn_c[which(kknn_c$eGFR_cyst>=90 & kknn_c$UACR_cat==2),]
data21 = kknn_c[which(kknn_c$eGFR_cyst>=60 &kknn_c$eGFR_cyst<90 & kknn_c$UACR_cat==0),]
data22 = kknn_c[which(kknn_c$eGFR_cyst>=60 &kknn_c$eGFR_cyst<90 & kknn_c$UACR_cat==1),]
data23 = kknn_c[which(kknn_c$eGFR_cyst>=60 &kknn_c$eGFR_cyst<90 & kknn_c$UACR_cat==2),]
data31 = kknn_c[which(kknn_c$eGFR_cyst>=45 &kknn_c$eGFR_cyst<60 & kknn_c$UACR_cat==0),]
data32 = kknn_c[which(kknn_c$eGFR_cyst>=45 &kknn_c$eGFR_cyst<60 & kknn_c$UACR_cat==1),]
data33 = kknn_c[which(kknn_c$eGFR_cyst>=45 &kknn_c$eGFR_cyst<60 & kknn_c$UACR_cat==2),]
data41 = kknn_c[which(kknn_c$eGFR_cyst>=30 &kknn_c$eGFR_cyst<45 & kknn_c$UACR_cat==0),]
data42 = kknn_c[which(kknn_c$eGFR_cyst>=30 &kknn_c$eGFR_cyst<45 & kknn_c$UACR_cat==1),]
data43 = kknn_c[which(kknn_c$eGFR_cyst>=30 &kknn_c$eGFR_cyst<45 & kknn_c$UACR_cat==2),]
data51 = kknn_c[which(kknn_c$eGFR_cyst>=15 &kknn_c$eGFR_cyst<30 & kknn_c$UACR_cat==0),]
data52 = kknn_c[which(kknn_c$eGFR_cyst>=15 &kknn_c$eGFR_cyst<30 & kknn_c$UACR_cat==1),]
data53 = kknn_c[which(kknn_c$eGFR_cyst>=15 &kknn_c$eGFR_cyst<30 & kknn_c$UACR_cat==2),]
data61 = kknn_c[which(kknn_c$eGFR_cyst<15 & kknn_c$UACR_cat==0),]
data62 = kknn_c[which(kknn_c$eGFR_cyst<15 & kknn_c$UACR_cat==1),]
data63 = kknn_c[which(kknn_c$eGFR_cyst<15 & kknn_c$UACR_cat==2),]

rbind(incidence(data11),
incidence(data12),
incidence(data13))

rbind(incidence(data21),
      incidence(data22),
      incidence(data23))

rbind(incidence(data31),
      incidence(data32),
      incidence(data33))

rbind(incidence(data41),
      incidence(data42),
      incidence(data43))

rbind(incidence(data51),
      incidence(data52),
      incidence(data53))

rbind(incidence(data61),
      incidence(data62),
      incidence(data63))

datalow_med = rbind(data11,data12,data21,data22,data31)
incidence(datalow_med)
