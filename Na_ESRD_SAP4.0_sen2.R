...
kknn_c$NA_INTERSALT = kknn_c$data.KAWASAKI
temp.mean = mean(kknn_c$NA_INTERSALT)
temp.sd = sd(kknn_c$NA_INTERSALT)
rm_outlier_KAWASAKI = which(kknn_c$NA_INTERSALT > temp.mean + 3*temp.sd | kknn_c$NA_INTERSALT < temp.mean - 3*temp.sd)
length(rm_outlier_KAWASAKI)
kknn_c = kknn_c[-rm_outlier_KAWASAKI,]
...
kknn_c$na_int_binary = ifelse(kknn_c$NA_INTERSALT < 2, "group1","group2")
...
#dilution regression
##calculate lambda
nd= which(!is.na(kknn_c$data.KAWASAKI2))
cal_dil = data.frame(first = kknn_c$NA_INTERSALT[nd],second = kknn_c$data.KAWASAKI2[nd])
...
m = lm(second~first,data=cal_dil)
...
sen2_a = list(sm5.1,sm5.2,sm5.3,sm5.4,lambda)
save(sen2_a,file = paste("sen2_res",i,".Rdata",sep=""))
